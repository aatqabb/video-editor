export function programPlaybackSyncPlugin() {
  return {
    ...
    transform(code, id) {
      ...
      const programMonitorCall = 'timelineClips={clips} playhead={playhead} projectSettings={projectSettings}'
      if (next.includes(programMonitorCall)) {
        next = next.replace(programMonitorCall, 'timelineClips={clips} tracks={tracks} playhead={playhead} projectSettings={projectSettings}')
      }
