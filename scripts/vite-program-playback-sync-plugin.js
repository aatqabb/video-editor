  ...
  return {
    ...
    transform(code, id) {
      ...
      if (!next.includes('timelineClips={clips} tracks={tracks}')) {
        ...
        if (programMonitorCallPattern.test(next)) next = next.replace(programMonitorCallPattern, 'timelineClips={clips} tracks={tracks}')
      }
      ...
      const newVideo = '<ProgramVideoMedia key={activeVideo.id} clip={activeVideo} playing={playing && !activeVideo.video?.freezeFrame} playhead={playhead} style={programVideoStyle} />'
      if (next.includes(oldVideo)) next = next.replace(oldVideo, newVideo)
      else if (!next.includes('<ProgramVideoMedia key={activeVideo.id}')) throw new Error('Program playback sync could not replace Program Monitor video element')
