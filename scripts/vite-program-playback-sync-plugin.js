  ...
  return {
    ...
    transform(code, id) {
      ...
      if (!next.includes('timelineClips={clips} tracks={tracks}')) {
        const programMonitorCallPattern = /timelineClips=\{clips\}(?=[\s\S]{0,220}?playhead=\{playhead\}[\s\S]{0,220}?projectSettings=\{projectSettings\})/
      ...
      const newVideoBranch = 'activeVideo.remoteUrl || activeVideo.localUrl || activeVideo.originalUrl || activeVideo.kind === \'image\' ? ('
      ...
      const oldTransitionBadge = '{clip.transition && <span className="transition-badge">↔</span>}'
      const transitionStartBadge = '{clip.transition && <span className="transition-start-badge" style={{ width: Math.max(12, Math.min(clip.duration * pixelsPerSecond, Number(clip.transition.duration || 0.5) * pixelsPerSecond)) }} title={`${clip.transition.type} · ${clip.transition.duration}s`}>↔</span>}'
      if (next.includes(oldTransitionBadge)) next = next.replace(oldTransitionBadge, transitionStartBadge)

      const oldHiddenButton = '<button className={track.hidden ? \'on\' : \'\'} onClick={() => toggleTrack(track.id, \'hidden\')}>◉</button>'
      const newHiddenButton = '<button className={track.hidden ? \'on track-hidden-toggle\' : \'track-hidden-toggle\'} aria-pressed={track.hidden} title={track.hidden ? \'Show layer in Program Monitor\' : \'Hide layer from Program Monitor\'} onClick={() => toggleTrack(track.id, \'hidden\')}>{track.hidden ? \'◎\' : \'◉\'}</button>'
      if (next.includes(oldHiddenButton)) next = next.replace(oldHiddenButton, newHiddenButton)

      next = next.replace('className={`track-control`}', 'className={`track-control`}')
      next = next.replace('className="track-control" style={{ height: trackHeight }}', 'className={`track-control ${track.hidden ? \'track-hidden\' : \'\'}`} style={{ height: trackHeight }}')
      next = next.replace('className={`track-lane ${track.locked ? \'locked\' : \'\'}`}', 'className={`track-lane ${track.locked ? \'locked\' : \'\'} ${track.hidden ? \'track-hidden\' : \'\'}`}')
