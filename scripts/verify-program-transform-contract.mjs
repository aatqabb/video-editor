...
const checks = [
  ...
  ['monitor component defines selection props', /selectedClipIds = \[\][\s\S]*setSelectedClipIds = \(\) => \{\}/, plugin],
  ['program clip click selects active clip', /setSelectedClipIds\(\[activeVideo\.id\]\)/, plugin],
  ['resize targets any active non-text video clip', /activeVideo\.id/, plugin],
  ['imported local and stock remote media share Program Monitor renderer', /clip\.remoteUrl \|\| clip\.localUrl \|\| clip\.originalUrl/, playback],
...
console.log('PASS program monitor live resize contract verified for imported/local and stock video clips.')
