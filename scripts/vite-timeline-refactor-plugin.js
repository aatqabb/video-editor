export function timelineRefactorPlugin() {
  return {
    ...
    transform(code, id) {
      ...
      // Do not seed a mysterious yellow marker in a new project.
      ...
      const scrubNew = `  const scrubFromEvent = (event) => {\n    if (event.target.closest('.timeline-clip')) return\n    // Premiere-style: a click positions the playhead. Merely moving/dragging the mouse over the timeline does not chase the pointer.\n    event.preventDefault()\n    setPlayhead(pointerToTime(event))\n  }`
      ...
      if (next.includes(dragSelectionNeedle)) {
        next = next.replace(dragSelectionNeedle, `    if (!selectedClipIds.includes(clip.id)) setSelectedClipIds([clip.id])\n    const rect = event.currentTarget.getBoundingClientRect()\n    dragInfoRef.current = { clipId: clip.id, offsetSeconds: (event.clientX - rect.left) / pixelsPerSecond }`)
      }
