...
export function TextWorkspace({ selectedTextClip, onAddText, onUpdateText, notify }) {
  ...
  const loadPcFonts = async () => {
    if (!window.queryLocalFonts) {
      notify('This browser does not expose installed fonts; common fonts are available')
      return
    }
    try {
  ...
  return (
    <div className="creative-panel text-workspace">
      ...
      <div className="creative-grid two">
        <label>Animation In<select value={animationIn} onChange={(event) => setAnimationIn(event.target.value)}>{['None', 'Fade In', 'Slide Up', 'Slide Left', 'Zoom In', 'Pop', 'Typewriter', 'Blur In'].map((item) => <option key={item}>{item}</option>)}</select></label>
        <label>Animation Out<select value={animationOut} onChange={(event) => setAnimationOut(event.target.value)}>{['None', 'Fade Out', 'Slide Down', 'Slide Right', 'Zoom Out', 'Shrink', 'Blur Out'].map((item) => <option key={item}>{item}</option>)}</select></label>
      </div>
...
export function TransitionWorkspace({ onApply, notify }) {
  ...
  const transitions = ['Cross Dissolve', 'Fade', 'Slide Left', 'Slide Right', 'Push', 'Zoom In', 'Zoom Out', 'Light Leak Warm', 'Light Leak Cool', 'Whip Pan', 'Flash', 'Blur Transition']

  return (
    <div className="creative-panel">
      <div className="creative-heading"><strong>TRANSITIONS</strong><span>Apply to selected video clip</span></div>
      <label className="creative-slider">Duration <input type="range" min="0.1" max="2" step="0.05" value={duration} onChange={(event) => setDuration(Number(event.target.value))} /><span>{duration.toFixed(2)}s</span></label>
      <div className="preset-grid">
        {transitions.map((transition) => <button key={transition} onClick={() => { onApply(transition, duration); notify(`${transition} applied`) }}>{transition}</button>)}
      </div>
    </div>
  )
}
...
export function EffectsWorkspace({ onApply, onReset, notify }) {
  ...
  return (
    <div className="creative-panel">
      <div className="creative-heading"><strong>VIDEO EFFECTS</strong><button onClick={() => { onReset(); notify('Effects reset') }}>Reset</button></div>
      <label className="creative-slider">Intensity <input type="range" min="0" max="100" value={intensity} onChange={(event) => setIntensity(Number(event.target.value))} /><span>{intensity}%</span></label>
      <div className="preset-grid">
        {effects.map((effect) => <button key={effect} onClick={() => { onApply(effect, intensity); notify(`${effect} applied`) }}>{effect}</button>)}
      </div>
    </div>
  )
}
...
const BUILT_IN_SFX = [
  { id: 'whoosh', name: 'Whoosh', duration: 0.7 },
  { id: 'impact', name: 'Impact Hit', duration: 0.5 },
  { id: 'pop', name: 'Pop', duration: 0.25 },
  { id: 'rise', name: 'Rise', duration: 1.2 },
  { id: 'glitch', name: 'Glitch', duration: 0.55 },
  { id: 'bass', name: 'Bass Drop', duration: 0.9 },
  { id: 'swipe', name: 'Swipe', duration: 0.45 },
  { id: 'boom', name: 'Boom', duration: 1 },
  { id: 'typing', name: 'Typing', duration: 0.8 },
  { id: 'notification', name: 'Notification', duration: 0.35 },
]
...
function synthTone(sfx) {
  ...
  const end = now + Math.min(1.5, sfx.duration)
  const low = ['impact', 'bass', 'boom'].includes(sfx.id)
  const high = ['pop', 'notification', 'typing'].includes(sfx.id)
  oscillator.type = sfx.id === 'glitch' ? 'square' : low ? 'sine' : 'triangle'
  oscillator.frequency.setValueAtTime(low ? 90 : high ? 720 : 240, now)
  oscillator.frequency.exponentialRampToValueAtTime(low ? 45 : high ? 420 : 80, end)
...
export function SfxWorkspace({ onAddSfx, notify }) {
  ...
  const filtered = useMemo(() => BUILT_IN_SFX.filter((sfx) => sfx.name.toLowerCase().includes(search.toLowerCase())), [search])

  const importOwnSfx = (file) => {
    if (!file) return
    const url = URL.createObjectURL(file)
    const audio = new Audio(url)
    audio.addEventListener('loadedmetadata', () => {
      onAddSfx({ id: `custom-${Date.now()}`, name: file.name, duration: Number.isFinite(audio.duration) ? audio.duration : 3, url, custom: true })
  ...
  return (
    <div className="creative-panel sfx-workspace">
      <div className="creative-heading"><strong>SFX LIBRARY</strong><button onClick={() => fileRef.current?.click()}>+ Import SFX</button></div>
        ...
        {filtered.map((sfx) => (
          <div
            ...
            onDragStart={(event) => {
              ...
              event.dataTransfer.setData('application/x-video-editor-sfx', JSON.stringify(sfx))
            }}
          >
            <button className="sfx-play" onClick={() => synthTone(sfx)}>▶</button>
            <span>{sfx.name}</span>
            <small>{sfx.duration.toFixed(2)}s</small>
            <button onClick={() => onAddSfx(sfx)}>Add</button>
          </div>
        ))}
      </div>
    </div>
